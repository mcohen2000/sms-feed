    ...
    <div className="w-full rounded-md bg-white text-black">
      ...
      <table className="text-left w-full">
        <thead className="table-header-group border-b-2">
            ...
            {/* <div className="flex items-center gap-4"> */}
                <th className="min-w-[100px] w-[100px] max-w-[100px] px-4 py-3">Date</th>
                <th className="truncate min-w-[400px] w-[400px] max-w-[400px] px-4 py-3">Body</th>
                <th className="min-w-[100px] w-[100px] px-4 py-3">Status</th>
                <th className="min-w-[100px] w-[100px] px-4 py-3"># Delivered</th>
            {/* </div> */}
            </tr>
        </thead>
      <tbody className="table-row-group">
        {allPosts ? (
            allPosts.map((post) => {
                console.log(post)
                return <PostQueueItem key={post.id} post={post} />
          })
          ) : (
              <li>Loading Posts...</li>
              )}
      ...
      <div className="flex items-center justify-center gap-6 py-4">
        ...
        {searchParams.page && parseInt(searchParams.page) > 1 ? (
            ...
            href={`/admin/dashboard?view=queue${searchParams.search ? `&search=${searchParams.search}`:""}${searchParams.sent ? `&sent=${searchParams.sent}`:""}&page=${
